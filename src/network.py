...
class GNet(nn.Module):
    ...
    def metric(self, logits, labels):
        ...
        layer_2_norm = torch.linalg.matrix_norm(self.out_l_2.weight)/ \
                    (self.out_l_2.weight.shape[0]*self.out_l_2.weight.shape[1])
        loss = F.nll_loss(logits, labels) + lambda_ * \
               (0.5*(layer_1_norm + layer_2_norm))
        # print('l_1.weight=', self.out_l_1.weight, self.out_l_1.weight.shape)
        # print('l_2.weight', self.out_l_2.weight, self. out_l_2.weight.shape)
